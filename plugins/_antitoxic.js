...
user.warn += 1
if (!(user.warn >= 3)) await m.reply(`🦈 *¡ALERTA DE PRESA TÓXICA!*\n\n*@${m.sender.split`@`[0]}*\n\n⚠️ Has usado lenguaje prohibido: (${isToxic})\n🎯 Advertencias: *${user.warn}/3*\n\n🦈 ¡MANTÉN LIMPIAS NUESTRAS AGUAS!`, false, { mentions: [m.sender] })}

if (user.warn >= 3) {
user.warn = 0
await m.reply(`🦈 *¡PRESA ELIMINADA!*\n\n*@${m.sender.split`@`[0]}*\n\n⚔️ ¡Has contaminado demasiado nuestras aguas!\n🌊 ¡SERÁS DEVORADO POR EL TIBURÓN!`, false, { mentions: [m.sender] })
user.banned = true
